            ...
            for dtype in dtype_list:
                ...
                for in_out in context_list:
                    for bs in tqdm(batchsize_list):
                        ...
                        runtime_decoder, single_layer_items = proj_decoder(config)
                        ...
                        #      round(single_layer_items["qkvo"]["memory"]/1024/1024/1024,
                        ...
                        #     prefill_layer_analysis[bs].append(
                        #         [in_out["in"], in_out["out"], dtype, bs, items["name"], round(items["operations"]/1e9, 2), round(items["memory"]/1024/1024/1024, 2),
                ...
                layer_projection_dict["prefill"][dtype].append(prefill_layer_projection)
                ...
                for in_out in context_list:
                    for bs in tqdm(batchsize_list):
                        ...
                        config = Config(device=device, type=type, dtype=dtype, pp=1, tp=1, hidden_size=hidden_size,
                                        ...
                                        is_decoding=True, kvcache_bucket=False)
                        runtime_decoder, single_layer_items = proj_decoder(config)
                        model_config = config.model_config
                        input_config = config.input_config
                        ...
                        #         [in_out["in"], in_out["out"], dtype, bs, items["name"], round(items["operations"]/1e9, 2), round(items["memory"]/1024/1024/1024, 2),
                        #          round(items["tops_roofline"]/1e12, 2), round(items["math_ai"], 2), items["bound"]])
                        # for items in single_layer_items["moe"]:
                        #     decoding_layer_analysis[bs].append(
                        #         [in_out["in"], in_out["out"], dtype, bs, items["name"], round(items["operations"]/1e9, 2), round(items["memory"]/1024/1024/1024, 2),
                        #          round(items["tops_roofline"]/1e12, 2), round(items["math_ai"], 2), items["bound"]])
                print("done!")
                projection_dict["decode"][dtype].append(decoding_projection)
                layer_projection_dict["decode"][dtype].append(layer_decoding_projection)
                # analysis_dict["decode"].append(decoding_layer_analysis)

            print_projection(projection_dict, model_name)
